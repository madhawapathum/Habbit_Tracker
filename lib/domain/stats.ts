import { Habit, HabitEntry, HabitHistory } from './types';
import { calculateCurrentStreak, calculateLongestStreak, isSameDay } from './streak';
 ...
 */
export function generateHabitHistory(
    habit: Habit,
    entries: HabitEntry[],
    referenceDate: Date = new Date()
): HabitHistory {
    ...
    const streakResult = calculateCurrentStreak(habitEntries, habit.targetDays, referenceDate);

    return {
        habitId: habit.id,
        streakValue: streakResult.value,
        streakStatus: streakResult.status,
        currentStreak: streakResult.displayCount,
